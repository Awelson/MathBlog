...
Note: \\( \mathbb{R}^+=[0,\infty) \\). This does form a partition of \\( \mathbb{R} \\). In this situation, \\( \hat{F} \\) maps \\( \\{x,-x\\}\to x^2, \ x\in \mathbb{R}^+ \\). Meanwhile, \\( \phi, \ x\to \\{x,-x\\}, \ x\in \mathbb{R} \\).
...
- First choose a random element from \\( a\in [x]_R \\). To be able to do this, we extract a function out of \\( \phi^{-1} \\).
- Apply \\( F \\) to \\( a \\)
- Apply \\( \phi \\) to \\( F(a) \\)
- Thus, \\( \hat{F}=\phi\circ F\circ \phi^{-1} \\)

![Capture.PNG](/MathBlog/assets/Capture19.PNG)

There is but a problem. Say \\( [x]_R \\) is an equivalence class of \\( A \\) and \\( x_1,x_2\in [x]_R \\), then on one instance we may have \\( \phi^{-1}([x]_R)=x_1 \\) and on another we may have \\( \phi^{-1}([x]_R)=x_2 \\). To make sure that \\( \hat{F} \\) is well defined, we need to show that \\( \phi(F(x_1))=\phi(F(x_2)) \\), i.e, that \\( F(x_1) \\) and \\( F(x_2) \\) are in the same equivalence class. This however is not guaranteed every single time.
