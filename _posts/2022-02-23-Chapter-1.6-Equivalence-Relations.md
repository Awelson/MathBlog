...
It doesn't matter which element we choose, since they are all in the same equivalence class, we know that \\( F \\) maps them to the same element. (This way of thinking does require us to use the axiom of choice).

> Defining \\( \hat{F} \\) the first way or the second way doesn't create two different functions. The points of \\( A/R \\) to the same points in \\( B \\) regardless of which definition we use.
