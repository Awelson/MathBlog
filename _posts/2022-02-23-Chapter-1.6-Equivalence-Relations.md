...
We shall say that a function \\( F:A\to A \\) is compatible if the above implication holds. For arguments sake let us assume that \\( F \\) is compatible, so \\( \hat{F} \\) is well defined. We still need to verify that (1) holds.
