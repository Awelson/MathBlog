...
> Theorem 1.6.4 Let \\( F:A\to A \\) be compatible, then there exists a unique function \\( \hat{F}:A/R\to A/R \\) such that \\( \hat{F}([x]_R)=[F(x)]_R \\).

Proof. Existence has already been shown, we only need to prove uniqueness.
