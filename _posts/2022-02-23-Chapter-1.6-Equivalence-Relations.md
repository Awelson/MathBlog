...
Proof. \\( \bigcup \\{[x]_R \ \vert \ x\in A \\}\subseteq A \\) is already obvious, so we only need to prove the converse. Note, since \\( R \\) is reflexive we have \\( x\in [x]_R \\) for all \\( x\in A \\). Choose an \\( a\in A \\), then we need to show that there is an \\( 
