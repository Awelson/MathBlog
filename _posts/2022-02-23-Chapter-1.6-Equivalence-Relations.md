...
is true. If we can eliminate all 3 possibilities, this would give rise to a contradiction. By assumption, \\( x_1<x_2 \ \Longrightarrow \ F(x_1)<F(x_2) \\) but this is not possible. Similarly, \\( x_2<x_1 \\) is not possible. Finally, \\( x_1=x_2 \\) breaks our assumption that \\( x_1,x_2 \\) are distinct.

Now we prove that \\( F(x)<F(y) \ \Longrightarrow \ x<y \\). We prove by contradiction again: Suppose that \\( F(x)<F(y) \\) but \\( \neg(x<y) \\). This means that one of \\( y<x \\) or \\( x=y \\) is true. Suppose \\( y<x \\) is true, then \\( F(y)<F(x) \\), but this is not possible due to trichotomy. Now suppose \\( x=y \\) is true, then \\( F(x)=F(y) \\), but this is also not possible due to trichotomy. \\( \Box \\)
