...
so equality holds if \\( F(\varphi^{-1}([x]_R)) \\) and \\( F(x) \\) are contained in the same equivalence class. But since \\( \varphi^{-1}([x]_R) \\) and \\( x \\) are by definition in the same equivalence class, our assumption that \\( F \\) is compatible implies that \\( F(\varphi^{-1}([x]_R)) \\) and \\( F(x) \\) are in the same equivalence class as needed.
