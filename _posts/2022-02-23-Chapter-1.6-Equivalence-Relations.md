...
It doesn't matter which element we choose, since they are all in the same equivalence class, we know that \\( F \\) maps them to the same element. (This way of thinking does require us to use the axiom of choice, having the ability to choose a random element from \\( [x]_R\in A/R \\) is equivalent to extracting a function out of \\( \phi^{-1} \\).
