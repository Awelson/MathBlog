...
Note: \\( \mathbb{R}^+=[0,\infty) \\). This does form a partition of \\( \mathbb{R} \\). \\( \hat{F} \\) in this situation would map \\( \\{x,-x\\}\to x^2 \\) for all \\( x\in \mathbb{R}^+ \\). Meanwhile, \\( \phi, \ x\to \\{x,-x\\} \\) for all \\( x\in \mathbb{R} \\).
