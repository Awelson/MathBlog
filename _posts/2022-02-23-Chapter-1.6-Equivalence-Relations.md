...
> Theorem 1.6.2 If \\( R\subseteq A\times A \\) is an RST relation, then \\( \\{[x]_R \ \vert \ x\in A \\} \\) forms a partition of \\( A \\).

Proof. 


\\( \bigcup \\{[x]_R \ \vert \ x\in A \\}\subseteq A \\) is already obvious, so we only need to prove the converse. Note, since \\( R \\) is reflexive we have \\( x\in [x]_R \\) for all \\( x\in A \\). Choose an \\( a\in A \\), then \\( a\in [a]_R\subseteq \bigcup \\{[x]_R \ \vert \ x\in A \\} \\). This proves the converse.
