...
> Lemma 1.6.2 If \\( R\subseteq A\times A \\) is an RST relation and \\( a_1,a_2\in A \\), then \\( a_1Ra_2 \\) iff \\( [a_1]_R=[a_2]_R \\).
...
Next we need to show that the relation generated from \\( \mathcal{C} \\) is equivalent to \\( R \\). The relation, \\( R' \\) generated from the partition is given as follows:

\\[ xR'y \quad \Longleftrigtharrow \quad x,y\in [z]_{R} \ \text{for some }z\in A \\]

but that is equivalent to \\( zRx \\) and \\( zRy \\), which by symmetry and transitivity gives \\( xRy \\) as we want.
