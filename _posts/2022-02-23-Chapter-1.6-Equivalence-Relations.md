...
- To transform \\( [x]_R\in A/R \\) into an element of \\( B \\), first choose randomly an element \\( a\in [x]_R \\). Then apply \\( F \\) to that element. Hence, \\( \hat{F}([x]_R)=F(a) \\) where \\( a \\) is some random element from \\( [x]_R \\). It doesn't matter which element we choose, since they are all in the same equivalence class, we know that \\( F \\) maps them to the same element. (This way of thinking does require us to use the axiom of choice)
