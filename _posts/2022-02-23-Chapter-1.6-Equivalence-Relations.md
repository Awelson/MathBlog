...
So we conclude that \\( \hat{F} \\) can only exist if: for any 2 \\( x_1,x_2\in A \\), \\( \ x_1Rx_2 \\) implies \\( F(x_1)RF(x_2) \\). Equivalently, if \\( x_1,x_2 \\) are in the same equivalence class, then so should \\( F(x_1),F(x_2) \\).
