...
Proof. Say Sal chooses some arbitrary sequence \\( (a_n)\to a \\) and gives it to Kwan, note. he has at his disposal, a number \\( N \\) so that \\( d(a_n,a)<1 \\) for all \\( n\geq N \\). Here is the strategy, he will define a number \\( r=\text{max}\\{d(a,a_1),\ldots,d(a,a_{N-1})\\}+1 \\). This number \\( r \\) will satisfy both
