...
- Kwan needs to be able to respond with a number \\( r>0 \\) that satisfies \\( \text{ran}(a_n)\subseteq B(a;r) \\), assuming he is able to, then since \\( B(a;r) \\) is bounded, so will be \\( \text{ran}(a_n) \\). 

Here is Kwan's strategy, he will respond with \\( r=\text{max}\\{d(a,a_1),\ldots,d(a,a_{N-1})\\}+1 \\). This number \\( r \\) is engineered to satisfy both
