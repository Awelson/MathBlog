...
Proof. Let \\( x=\text{sup }A \\), if \\( x\in A \\), then we are done because \\( (x,x,\ldots)\subseteq A \\) and \\( (x,x,\ldots)\to x \\) as needed. Let us suppose \\( x\notin A \\). For any \\( n \\), we know that \\( x-\frac{1}{n} \\) cannot be an upper bound of \\( A \\). Thus, for each \\( n \\), there has to exist an \\( a_n\in A \\) such that \\( x-\frac{1}{n}<a_n<x \\) [we write \\( a_n<x \\) instead of \\( a_n\leq x \\) because \\( x\notin A \\), so \\(a_n=x \\) is impossible]. 

I claim that \\( (a_n)\to x \\). Let an \\( \epsilon>0 \\) be chosen by Sal, Kwan replies by choosing an \\( N\geq \frac{1}{\epsilon} \\). Check that

\\[ x-\epsilon<x-\frac{1}{n}<a_n, \quad \text{for all }n\geq N. \\]

With a bit more work, we obtain \\( x-a_n=\|x-a_n\|<\epsilon \\) for all \\( n\geq N \\). Thus, \\( (a_n)\to x \\) as desired. 
