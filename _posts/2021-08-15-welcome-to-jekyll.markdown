...
Step 2 is equivalent to showing that there is no upper bound that is less than \\( x \\), i.e, we need to show that \\( c<x \ \Longrightarrow \ c\text{ is not an upper bound of }A \\). Reformulating step 2 this way will make things much easier (trust me).
