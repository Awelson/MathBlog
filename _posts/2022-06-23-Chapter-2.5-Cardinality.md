...
> Definition. We say that a set \\( A \\) is finite if there exists an \\( n\in \omega \\) such that \\( n\approx A \\) holds. Further if \\( n\approx A \\) then we say that \\( A \\) has a cardinality of \\( n \\).

There is one issue with this definition, that is, we could maybe have two distinct \\( n,m\in \omega \\) such that \\( n\approx m\approx A \\). So what is the cardinality of \\( A \\)? What is its size? is it \\( n \\) or \\( m \\)? Now intuitively \\( n\approx m \\) is impossible if they are distinct numbers, but the proof of this is actually extremely complicated. It relies on the following lemma:

> Theorem 2.5.5 [The pigeonhole principle] Let \\( n\in \omega \\). If \\( f:n\to n \\) is injective, then it is also surjective.
...
The proof is super tedious so I will omit it. I will however show how this lemma can be used to prove that \\( m\not\approx n \\) for distinct \\( n,m \\):

Proof. Suppose \\( n<m \\). Let \\( f:m\to n \\) be an arbitrary injective function. Since \\( n\subseteq m \\) we also have \\( f:m\to m \\). The lemma tells us that \\( f:m\to m \\) is surjective. Choose some \\( y\in m \\) that is not in \\( n \\) (of course we can do this). Then there exists an \\( x\in m \\) such that \\( f(x)=y \\). The claim that \\( f:m\to n \\) is false now as \\( f(x)\notin n \\). Thus there can't exist an injective (hence bijective) function which maps \\( m\to n \\). \\( \Box \\)
