...
Proof. Suppose there is a bijection from \\( f:A\to \mathcal{P}(A) \\), then we can construct a bijection \\( e:A\to \\{0,1\\}^A \\) simply by composing \\( f \\) with the bijection from lemma 2.5.1. Let \\( s\in \\{0,1\\}^A \\) be defined as follows: \\( s(x)=\neg([e(x)](x)) \\) (where \\( \neg(0)=1 \\) and \\( \neg(1)=0 \\)). By bijectivity of \\( e \\), there exists a \\( y\in A \\) such that \\( e(y)=s \\). 
