...
so what is the 1st decimal point of \\( f(n) \\)? is it 9 or 0? When we encounter a situation like this, just choose one side. For example let us choose 9, then when we encounter the same situation, we remain consistent with our choice and choose 9 again. 

The real number \\( z \\) is constructed as follows:

\\[ z=0.g(1)g(2)g(3)\cdots \\]

We claim that \\( z \\) is not in the image of \\( f \\). Suppose it is, then \\( f(n)=z \\) for some \\( n\in \omega \\). The nth decimal place of \\( z \\) is \\( g(n) \\), but we know that the nth decimal place of \\( f(n) \\) cannot equal \\( g(n) \\), so \\( f(n) \\) and \\( z \\) can't be the same number.
