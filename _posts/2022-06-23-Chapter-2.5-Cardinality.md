...
There is one issue with this definition, that is, we could maybe have two distinct \\( n,m\in \omega \\) such that \\( n\approx m\approx A \\). So what is the cardinality of \\( A \\)? What is its size? is it \\( n \\) or \\( m \\)? Now intuitively \\( n\approx m \\) is impossible if they are distinct numbers, but the proof of this is actually extremely complicated. 
