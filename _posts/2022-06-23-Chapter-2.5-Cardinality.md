...
In particular, \\( [[0,n]]=n^+ \\) for all \\( n\in \omega \\). We shall use the 'list' form to describe numbers now since that is more appropriate (we are however excluding 0 since that can't be interpreted in 'list' form). Now getting back to the proof; it relies on the following:
