...
Proof. Let \\( A \\) be a finite set and \\( B\subset A \\). Assume there exists a bijection \\( f:A\to B \\). Since \\( A \\) is finite, there is a bijection \\( g:A\to n \\) for some \\( n\in \omega \\). Consider the function \\( \kappa=g\circ f\circ g^{-1}:n\to n \\). As a composition of injective functions, it is injective. We can't use the same argument to show that it is surjective (why?), instead we rely on the pigeonhole principle to infer that \\( \kappa \\) is surjective as well.

Notice that \\( \text{Im }\kappa=g\[B\] \\). We can choose an \\( a\in A-B \\), then \\( g(a)\notin \text{Im }\kappa \\). But this contradicts \\( \text{Im }\kappa=n \\), i.e, the surjectivity of \\( \kappa \\). \\( \Box \\) 
