...
2. needs to show two things to win: that \\( x\in C \\) and that \\( x\notin A \\). He may use the fact that \\( {A\subseteq B}^1 \\).

(2) The following game needs to be won:

1. An \\( x \\) is chosen satisfying the following: if Kwan were to choose any \\( b\in B \\), then \\( x \\) would be in \\( b \\).
2. Sal can pick any \\( a\in A \\), to win, Kwan needs to show that \\( x\in a \\). He may use the fact that \\( A\subseteq B \\).

Whatever \\( a \\) Sal chooses is an element of \\( B \\) since \\( a\in A\subseteq B \\). Thus, Kwan can choose the element \\( a\in B \\), and it would be true that \\( x\in a \\). \\( \Box \\)
