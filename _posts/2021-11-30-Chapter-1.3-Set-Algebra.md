...
\\[ \begin{align*} 1. \ A\subseteq B \quad &\Longrightarrow \quad A\cup C\subseteq B\cup C \\ 2. \ A\subseteq B \quad &\Longrightarrow \quad A\cap C \subseteq B\cap C \\ 3. \ A\subseteq B \quad &\Longrightarrow \quad \bigcup A\subseteq \bigcup B. \\]

Proof. (1) We need to win the following game:

1. Sal chooses either an \\( x\in A \\) or an \\( x\in C \\)
2. Kwan has two win conditions: to show that either \\( x\in B \\) or that \\( x\in C \\). He is allowed to use the fact that \\( {A\subseteq B}^1 \\).
