...
(2) Now we show that \\( h \\) is an acceptable function. Both, \\( \text{dom }h\subseteq \omega \\) and \\( \text{ran }h\subseteq A \\) are clear. We shall check (i) first. If \\( 0\in \text{dom }h \\) then there must exist an acceptable function \\( v \\) such that \\( <0,v(0)>\in h \\). We know that \\( v(0)=a \\) so \\( h(0)=a \\) as needed.
