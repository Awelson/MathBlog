...
We can form the collection \\( \mathcal{K} \\) of all possible acceptable functions. This can be done by filtering out what we want from the collection of the subsets of \\( \omega\times A \\), i.e, \\( \mathcal{P}(\omega\times A) \\). Our claim is that \\( h=\bigcup \mathcal{K} \\). Notice that this means \\( <n,y>\in h \\) iff we can find some \\( v\in \mathcal{K} \\) for which \\( v(n)=y \\). Another thing to note is that for an acceptable function \\( v \\) with \\( n\in \text{dom }v \\), we have \\( <n,v(n)>\in v\subseteq \bigcup \mathcal{K}=h \\), in short, \\( <n,v(n)>\in h \\).
...
If we can show that \\( T=\omega \\), then any \\( n \\) we pick from \\( \text{dom }h\subseteq \omega \\) (that \\( \text{dom }h \\) is a subset of \\( \omega \\) should be obvious) should satisfy \\( (\exists! y)<n,y>\in h \\), meaning \\( h \\) is a function as needed.
