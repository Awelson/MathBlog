...
(1) The only reason why \\( h \\) can fail to be a function is if there is \\( v_1,v_2\in h \\) such that \\( v_1(n)\neq v_2(n) \\) for some \\( n\in \omega \\). This means that both \\( <n,v_1(n)>,<n,v_2(n)\in h \\).
