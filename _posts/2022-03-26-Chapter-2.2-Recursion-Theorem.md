...
We now check (ii). Suppose \\( n^+\in \text{dom }h \\), then we can find an acceptable function \\( v \\) such that \\( <n^+,v(n^+)>\in h \\). Now \\( v(n^+)=F(v(n))=F(h(n)) \\) (because \\( <n,v(n)>\in h \\) and \\( h \\) is a function).