...
First we show \\( <0,a>\in h \\). To that end, let Sal choose some \\( v\in \mathcal{K} \\), now we need to show that \\( <0,a>\in v \\), this is obvious though. Next, suppose that \\( <n,y>\in h \\), we need to show that \\( <n^+,F(y)>\in h \\). Let Sal choose some \\( v\in \mathcal{K} \\), we know that \\( <n,y>\in v \\) (because \\( <n,y>\in h \\)) so \\( <n^+,F(y)\in v \\) is obvious. This shows that \\( h\in \mathcal{K} \\).
