...
Proof. Choose \\( x\in \mathcal{P}(A) \cap \mathcal{P}(B) \\), then \\( x\subseteq A \land x\subseteq B \\), check that \\( x\subseteq A\cap B \\) is true, thus \\( x\in \mathcal{P}(A\cap B) \\). The reverse implication is similar. \\( \Box \\)
