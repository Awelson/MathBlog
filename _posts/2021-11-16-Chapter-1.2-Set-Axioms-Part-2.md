...
Kwan starts with the obvious fact that \\( x\subseteq a \\). Next, since \\( a\in B \\), Kwan invokes theorem 1.2.1, showing that \\( a\subseteq \bigcup B \\). Thus, \\( x\subseteq a\subseteq \bigcup B \\), and by transitivity of the subset relation we have \\( x\subseteq \bigcup B \\) as desired. \\( \Box \\)
