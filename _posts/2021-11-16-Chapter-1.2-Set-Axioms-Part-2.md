...
\\[ \bigcup \\{A,B,C\\}=A\cup B\cup C, \quad \bigcup \\{A,B,C,D\\}=A\cup B\cup C\cup D, \quad and so on...  \\]
...
Here are some more tid bits regarding unions:

> Theorem 1.2.1 Let \\( A\neq \varnothing \\), then if \\( x\in A \\) we must have \\( x\subseteq \bigcup A \\).
