...
exists. Alternatively we could have done \\( A\cap B=\\{ x\in B \ \vert \ x\in A \\} \\), but this doesn't matter. What about an infinite intersection? If \\( A=\\{A_1,A_2,\ldots\\} \\) then it is reasonable to assume that we can define \\( \bigcap A \\) so that \\( x\in \bigcap A \ \Leftrightarrow \ forall a\in A, \ x\in a \\), i.e, it is the set containing those elements which are contained in every member of \\( A \\). Assuming that \\( A\neq \empty \\), the existence of \\( \bigcap A \\) is also a result of the axiom of specification:

Proof. First, we need to choose a set, since \\( A\neq \empty \\), we can pick an element \\( y\in A \\), this will be the set to use. Our condition can then be \\( \forall a\in A, \ x\in a \\). All in all,

\\[ \bigcap A=\\{x\in y \ \vert \ \forall a\in A, \ x\in a\\} \\]

exists (by axiom of specification). \\( \Box \\)

Why did we ignore the case where \\( A=\empty \\), is there something so wrong with \\( \bigcap \empty \\)? Yes! it's existence will result in a contradiction:

> Claim. \\( \bigcap \empty \\) contains everything, thus it should not exist (remember we have already shown that there cannot be a set which contains everything).

Proof. Suppose not, i.e, that there exists a set \\( B \\) such that \\( B\notin \bigcap \empty \\). Since

\\[ x\in \bigcap \empty \quad \Longleftrightarrow \quad \forall A\in \empty, \ x\in A \\]

we must have \\( \exists A\in \empty, \ B\notin A \\).
