...
Suppose Sal is able to find some other \\( x'\in \text{dom }R'\circ R \\) for which \\( x'(R'\circ R)z \\), then this would mean that there exists a \\( y' \\) such that \\( xRy' \ \land y'R'z \\). Since \\( R' \\) is single rooted, we have \\( y'=y \\), continuing, since \\( R \\) is single rooted, we have \\( x=x' \\). Thus, Sal isn't able to choose a different \\( x' \\), Kwan wins.
