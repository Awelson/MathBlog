...
2) Assume that there is a function \\( H \\) for which \\( F\circ H=I_B \\). We need to show that \\( \text{Im }F=B \\), it suffices to show that \\( B\subseteq \text{Im }F \\) because the converse is already a given. Sal picks an element \\( y\in B \\), then \\( F(H(y))=y \\) by assumption
