...
but clearly there is no \\( x \\) satisfying \\( x\in \varnothing \\). However, notice that \\( x^{5x} \\)
...
Proof. This is clear from the previous example, but let us provide an explicit example: Let \\( F=\\{<1,3>,<2,3>\\} \\), then
\\[ F^{-1}=\\{<3,1>,<3,2>\\} \\]
