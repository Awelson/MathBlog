...
- There exists a function \\( G \\) with domain \\( B \\) (a "left inverse") such that \\( G\circ F \\) is the identity function \\( I_A \\) on \\( A \\) iff \\( F \\) is one-to-one.
...
Keep in mind that \\( F(x)\in \text{Im }F \\) for all \\( x\in A \\).
