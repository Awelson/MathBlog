...
Notice that by definition, \\( B\subseteq Z \\) for all \\( Z\in P=\\{X\subseteq A \ \vert \ F(X)\subseteq X \\} \\). Thus, \\( F(B)\subseteq F(Z)\subseteq Z \\) for all \\( Z\in P \\). This means \\( F(B)\subseteq \bigcap P=B \\) as needed.
