...
- There exists a function \\( H:B\to A \\) (a "right inverse") such that \\( F\circ H \\) is the identity function \\(I_B \\) on \\( B \\) iff \\( F \\) is surjective.
...
so \\( F \\) is one-to-one. For the converse, assume \\( F \\) is one-to-one, then \\( F^{-1} \\) is a function by theorem 1.5.3, further \\( F^{-1} \\) has domain \\( \text{Im }F \\) 
