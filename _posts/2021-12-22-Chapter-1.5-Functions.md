...
How \\( H \\) is "extracted" from \\( R \\) is by removing the elements which contribute towards the mapping of an element of the domain to more than one point. In this case, \\( R \\) contains both \\( <1,4> \\) and \\( <1,2> \\) so getting rid of one of them will turn \\( R \\) into a function, in doing so, we are "choosing" which one of them survives.  

How does this help us? Notice that \\( F^{-1} \\) is not a function this time unlike (1), it is however a relation which we can extract a function out of. Let \\( H \\) be the function created from extraction of \\( F^{-1} \\), then \\( \text{dom }H=\text{dom }F^{-1}=\text{Im }F=B \\). Further, for any \\( y\in B \\), \\( <y,H(y)>\in F^{-1} \\), 
