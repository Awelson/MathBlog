...
If \\( x\in A \\) then it is implicitly implied that \\( x\in \text{dom }R \\) so there exists a \\( y\in \text{im }R \\) such that \\( xRy \leftrightarrow yR^{-1}x \\). More specifically, \\( y\in R[A] \\), so \\( x\in R^{-1}[R[A]] \\) as needed (the converse doesn't rely on single-rooted property). \\( \Box \\)  
