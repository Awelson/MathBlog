...
Addition can be thought of as the function \\( f:\mathbb{R}\times \mathbb{R}\to \mathbb{R} \\), it assigns an ordered pair of real numbers \\( <a,b> \\), to the real number \\( a+b \\), so for example, \\( f \\) maps \\( <4,5>\to 9 \\), and so on.
