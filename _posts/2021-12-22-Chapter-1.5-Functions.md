...
First note, if Kwan is able to find an \\( x\in A-B \\) for which \\( xFy \\), then this is equivalent to the win condition. First, Kwan notes that since \\( y\in F[A] \\), he is able to find an \\( x\in A \\) for which \\( xFy \\). Next, he will show that it is impossible for the \\( x\in A \\) he found to be an element of \\( B \\) as well, for if it were true, then that would imply \\( y\in F[B] \\) (a contradiction). Thus, the \\( x \\) that Sal found is an element of \\( A-B \\) as needed. \\( \Box \\)
...
If Kwan can show that he is able to find an \\( x\in A \\) for which \\( xFy \\), then this is 1/2 of the job done, for the other half, he needs to show that it is impossible to find an \\( x\in B \\) for which \\( xFy \\). Since \\( y\in F[A-B] \\), Kwan notes that he is able to find an \\( x_1\in A-B \\) for which \\( x_1Fy \\), this is the first half done ( since \\( A-B\subseteq A \\) ). For the second half, suppose Sal is able to find an \\( x_2\in B \\) for which \\( x_2Fy \\), then because \\( F \\) is single rooted, we have \\( x_1=x_2 \\) but this is a contradiction because \\( x_1\in A-B \\) but \\( x_2\in B \\).
