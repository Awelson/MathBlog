...
so \\( F \\) is one-to-one. For the converse, assume \\( F \\) is one-to-one, then \\( F^{-1} \\) is a function by theorem 1.5.3, further \\( F^{-1} \\) has domain \\( \text{Im }F \\). It has the instructions to transform elements coming from \\( \text{Im }F \\), our idea is to make it so that \\( G \\) copies the same instructions, that is to say, we shall let \\( G(x)=F^{-1}(x) \\) for all \\( x\in \text{Im }F \\). The problem is that the domain of \\( G \\) is set to equal \\( B \\) and since \\( \text{Im }F\subseteq B \\), there might very well be elements of \\( B \\) which are not in \\( \text{Im }F \\) and \\( G \\) does not yet know how to transform these elements.

To remedy this problem, notice that since \\( A \\) is non-empty, we can choose a point \\( a\in A \\), then we can make it so that \\( G(x)=a \\) for all \\( x\in B-\text{Im }F \\). All in all,
...
Now we need to verify that \\( \[G\circ F\](x)=x \\) for all \\( x\in A \\):

\\[ \[G\circ F\](x)=G(F(x))=F^{-1}(F(x))=x \\] 

Keep in mind that \\( F(x)\in \text{Im }F \\) for all \\( x\in A \\). Finally, we simply assign \\( A \\) as the codomain for \\( G \\), but before we can do that we need to verify that \\( \text{Im }G\subseteq A \\). In other words, we need to show that \\( G(x)\in A \\) for all \\( x\in B \\). This is obvious when we observe that \\( F^{-1} \\) has image \\( \text{dom }F=A \\).
