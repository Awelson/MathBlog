...
Suppose \\( y\in F[A] \cup F[B] \\), then we have either \\( y\in F[A] \\) or \\( y\in F[B] \\). To win, we need to show that there exists an \\( x\in A\cup B \\) for which \\( xFy \\). WLOG, suppose \\( y\in F[A] \\), then there exists an \\( x\in A \\) for which \\( xFy \\)
