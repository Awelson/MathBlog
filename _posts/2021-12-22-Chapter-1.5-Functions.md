...
> Theorem 1.5.2 If \\( F \\) is a function, then \\( F^{-1}[A]=\\{x\in \text{dom }F \ \vert \ \exists y\in A, \ xFy\\} \\). It is the set containing all the elements which are assigned to an element of \\( A \\) 
