...
Example: The function \\( f(x)=x^2 \\) with domain \\( \mathbb{R} \\) is not injective because \\( 4\in \text{im }f \\) but \\( f(-2)=f(2)=4 \\), there are 2 elements in the domain of \\( f \\) which maps to 4. It turns out that if we change the domain of \\( f \\) into \\( [0,\infty) \\) instead then
