...
It is at this point where I would like to propose a different way for thinking about of functions and relations to make our proofs more readable. We can think of a relation \\( R \\) as a collection of arrows from \\( \text{dom }R \\) into \\( \text{im }R \\)

> Lemma 1.5.12 For any single rooted relation \\( R \\) we have \\( (R^{-1}\circ R)[A]=A \\).

Proof. We know \\( (R^{-1}\circ R)[A]=R^{-1}[R[A]] \\) from a previous theorem. Now suppose \\( x\in R^{-1}[R[A]] \\) then there exists a \\( y\in R[A] \\) such that \\( yR^{-1}x \\), in other words, \\( xRy \\). Since \\( y\in R[A] \\), there must exist an \\( x'\in A \\) such that \\( x'Ry \\), but because \\( R \\) is single rooted, we have \\( x=x' \\) so \\( x\in A \\) as needed.

If \\( x\in A \\) then it is implicitly implied that \\( x\in \text{dom }R \\) so there exists a \\( y\in R[A] \\) such that \\( xRy \leftrightarrow yR^{-1}x \\), so \\( x\in R^{-1}[R[A]] \\) as needed (the converse doesn't rely on single-rooted property). \\( \Box \\)  
