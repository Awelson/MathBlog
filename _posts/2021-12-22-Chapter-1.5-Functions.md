...
> Theorem 1.5.3 \\( F \\) is a single-rooted relation iff \\( F^{-1} \\) is a function. Similarly, \\( F \\) is a function iff \\( F^{-1} \\) is a single-rooted relation.
...
Proof. First we will show that \\( F\circ G \\) is a function. Suppose Sal disagrees, he asserts that he has found an \\( x\in \text{dom }F\circ G \\) for which \\( x(F\circ G)z \\) and \\( x(F\circ G)z' \\) with \\( z\neq z' \\). Kwan's win condition is to prove that Sal is lying. 
...
so by definition, \\( <x,F(G(x))\in F\circ G \\). This also proves that \\( (F\circ G)(x)=F(G(x)) \\) for \\( x \\) in its domain. We also need to prove the reverse, that if \\( x\in \text{dom }F\circ G \\), then both \\( x\in \text{dom }G \\) and \\( G(x)\in \text{dom }F \\).
