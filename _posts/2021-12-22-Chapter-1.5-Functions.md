...
Proof. 1) We shall prove \\( F(B)=B \\) only. For \\( C \\) the proof is similar. First notice that if we manage to prove \\( F(B)\subseteq B \\) this would imply \\( F(B)\in \\{X\subseteq A \ \vert \ F(X)\subseteq X \\} \\) and hence \\( B\subseteq F(B) \\) so our job is done.

Notice that by definition, \\( B\subseteq Z \\) for all \\( Z\in P=\\{X\subseteq A \ \vert \ F(X)\subseteq X \\} \\). Thus, \\( F(B)\subseteq F(Z)\subseteq Z \\) for all \\( P \\) in the set. This means \\( F(B)\subseteq \bigcap P=B \\) as needed.
