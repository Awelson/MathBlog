...
Proof. 1) We shall prove \\( F(B)=B \\) only. For \\( C \\) the proof is similar. First notice that if we manage to prove \\( F(B)\subseteq B \\) this would imply \\( F(F(B))\subseteq F(B) \\) and hence \\( F(B)\in \\{X\subseteq A \ \vert \ F(X)\subseteq X \\} \\). Thus, \\( B\subseteq F(B) \\).
