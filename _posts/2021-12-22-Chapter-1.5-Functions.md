...
Proof. 1) Suppose \\( y\in F[A\cup B] \\), then by definition there exists an \\( x\in A\cup B \\) for which \\( xFy \\). To win, we must show either \\( y\in F[A] \\) or \\( y\in F[B] \\). Since \\( x\in A\cup B \\) Kwan concludes that \\( x \\) must either be in \\( A \\) or in \\( B \\). If \\( x\in A \\), then we can conclude \\( y\in F[A] \\), alternatively, if \\( x\in B \\), then \\( y\in F[B] \\).

Suppose \\( y\in F[A] \cup F[B] \\), then we have either \\( y\in F[A] \\) or \\( y\in F[B] \\). To win, we need to show that there exists an \\( x\in A\cup B \\) for which \\( xFy \\). If \\( y\in F[A] \\), then there exists an \\( x\in A \\) for which \\( xFy \\), this is a win. Alternatively, if \\( y\in F[B] \\), then there exists an \\( x\in B \\) for which \\( xFy \\), this is also a win.

2) Suppose \\( y\in F[A\cap B] \\), then there exists an \\( x\in A\cap B \\) such that \\( xFy \\). Kwan extracts from \\( x\in A\cap B \\) that \\( x\in A \\), and hence that \\( y\in F[A] \\), at the same time he may also obtain \\( x\in B \\), and hence that \\( y\in F[B] \\). Thus, \\( y\in F[A] \cap F[B] \\) as needed. 

We will clarify why the converse is not true. Having \\( y\in F[A] \cap F[B] \\) means that \\( \exists x_1\in A, \ xFy \\) and \\( \exists x_2\in B, \ xFy \\). Meanwhile, we need to prove that \\( \exists x\in A\cap B \\) for which \\( xFy \\). This is not necessarily the case since it might be that \\( x_1\neq x_2 \\). Notice that if \\( F \\) is single-rooted, then \\( x_1=x_2 \\), in which case, equality holds just as in 1). \\( \Box \\)
