...
The fact that Sal chose a \\( z\in \text{im }R'\circ R \\) means that there exists an \\( x\in \text{dom }R'\circ R \\) such that \\( x(R'\circ R)z \\), so Kwan presents this \\( x \\) and the game proceeds. Before moving on, notice that since \\( x(R'\circ R)z \\), there must exists a \\( y \\) such that \\( xRy \ \land yR'z \\). Suppose Sal is able to find some other \\( x'\in \text{dom }R'\circ R \\) for which \\( x'(R'\circ R)z \\), then this would mean that there exists a \\( y' \\) such that \\( xRy' \ \land y'R'z \\).
