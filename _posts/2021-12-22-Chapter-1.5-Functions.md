...
> Theorem 1.5.3 If \\( F \\) is a relation, then \\( F^{-1} \\) is a function iff \\( F \\) is single-rooted. Also, a relation \\( F \\) is a function iff \\( F^{-1} \\) is single-rooted.
...
Proof. First notice that since \\( F \\) is a one-to-one function, then by theorem 1.5.3, \\( F^{-1} \\) is a function, so the notation makes sense. If \\( x\in \text{dom }F \\), then \\( <x,F(x)>\in F \\) and so \\( <F(x),x>\in F^{-1} \\), thus, \\( F^{-1}(F(x))=x \\).

Now suppose \\( y\in \text{ran }F \\), then \\( y\in \text{dom }F^{-1} \\), theorem 1.5.3 guarantees that \\( F^{-1} \\) is one-to-one so we can apply the first part of theorem 1.5.4, this gives \\( (F^{-1})^{-1}(F^{-1}(y))=y \\) but since \\( (F^{-1})^{-1}=F \\) we are done. \\( \Box \\)
