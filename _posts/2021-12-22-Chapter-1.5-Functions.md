...
To prove the domain claim, suppose Sal has chosen an \\( x \\) such that both \\( x\in \text{dom }G \\) and \\( G(x)\in \text{dom }F \\), then we need to show that \\( x\in \text{dom }F\circ G \\).
