...
Proof. Suppose \\( <x,y>\in (F\circ G)^{-1} \\), then

\\[ <y,x>\in F\circ G, \ \Longrightarrow \ \exists t, \ yGt \ \land tFx, \ \Longrightarrow \ \exists t, \  xF^{-1}t \ \land \ tG^{-1}y, \ \Longrightarrow \ <x,y>\in G^{-1}\circ G^{-1}. \quad \Box \\]
