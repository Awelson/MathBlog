...
> Lemma 1.5.12 For any relation \\( R \\) we have \\( (R^{-1}\circ R)[A]=A \\).

Proof. We know \\( (R^{-1}\circ R)[A]=R^{-1}[R[A]] \\) from a previous theorem. Now suppose \\( x\in R^{-1}[R[A]] \\) then there exists a \\( y\in R[A] \\) such that \\( yR^{-1}x \\). Thus, it must be that \\( xRy \\), since \\( y\in R[A] \\), we must also have \\( x\in A \\). 

The reverse is similar, if \\( x\in A \\) then \\( \exists y\in R[A], \ xRy \\). Thus, it must be that \\( yR^{-1}x \\) so \\( x\in R^{-1}[R[A]] \\) as needed. \\( \Box \\) 
