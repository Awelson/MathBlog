...
Proof. 1) Suppose \\( y\in F[A\cup B] \\), then by definition there exists an \\( x\in A\cup B \\) for which \\( xFy \\). To win, we must show either \\( y\in F[A] \\) or \\( y\in F[B] \\). Since \\( x\in A\cup B \\) Kwan concludes that \\( x \\) must either be in \\( A \\) or in \\( B \\), if \\( x\in A \\), then \\( y\in F[A] \\) because \\( xFy \\), similarly, if \\( x\in B \\), then \\( y\in F[B] \\).
