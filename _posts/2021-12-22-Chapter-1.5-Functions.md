...
Proof. First notice that since \\( F \\) is one-to-one, then by theorem 1.5.3, \\( F^{-1} \\) is a function, so the notation makes sense. If \\( x\in \text{dom }F \\), then \\( <x,F(x)>\in F \\) and so \\( <F(x),x>\in F^{-1} \\), thus, \\( F^{-1}(F(x))=x \\).
