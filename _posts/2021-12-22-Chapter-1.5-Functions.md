...
\\[ F^{-1}=\\{<3,1>,<3,2>\\} \\]
fails to be function. It is however a relation, so we can talk about the image of \\( F^{-1} \\) even if it is not a function. \\( \Box \\)

> Lemma 1.5.1 Let \\( F \\) be a function, then \\( \text{dom }F=\text{im }F^{-1} \\).
...
> Claim. If \\( F \\) is a function, then \\( F^{-1}[A]=\\{x\in \text{dom }F \ \vert \ \exists y\in A, \ xFy\\}.

Proof. By definition, \\( F^{-1}[A]=\\{ x\in \text{im }F^{-1} \ \vert \ \exists y\in A, \ xF^{-1}y\\} \\) but \\( xF^{-1}y \\) is equivalent to \\( xFy \\) and we will later show that \\( \text{im }F^{-1}=\text{dom }F \\).
