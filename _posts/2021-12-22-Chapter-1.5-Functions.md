...
\\[ F^{-1}=\\{<3,1>,<3,2>\\} \\]
fails to be function. It is however a relation, so we can talk about the image of \\( F^{-1} \\) eventhough it is not a function. \\( \Box \\)
...
> Theorem 1.5.2 If \\( F \\) is a function, then \\( F^{-1}[A]=\\{x\in \text{dom }F \ \vert \ \exists y\in A, \ xFy\\} \\).
