...
Proof. 1) We shall prove \\( F(B)=B \\) only. For \\( C \\) the proof is similar. First notice that if we manage to prove \\( F(B)\subseteq B \\) then our job is done; this is because we would have \\( F(B)\in \\{X\subseteq A \ \vert \ F(X)\subseteq X \\} \\) and hence \\( B\substeq F(B) \\).
