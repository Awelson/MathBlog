...
The fact that Sal chose a \\( z\in \text{im }R'\circ R \\) means that there exists an \\( x\in \text{dom }R'\circ R \\) such that \\( x(R'\circ R)z \\), so Kwan finds and presents this \\( x \\) and the game proceeds. Before moving on, notice that since \\( x(R'\circ R)z \\), there must exists a \\( y \\) such that \\( xRy \ \land yR'z \\). 

Suppose Sal is able to find some other \\( x'\in \text{dom }R'\circ R \\) for which \\( x'(R'\circ R)z \\), then this would mean that there exists a \\( y' \\) such that \\( xRy' \ \land y'R'z \\). Since \\( R' \\) is single rooted, we have \\( y'=y \\), continuing, since \\( R \\) is single rooted, we have \\( x=x' \\). Thus, Sal isn't able to choose a different \\( x' \\), Kwan wins. \\( \Box \\)

The above theorem tells us that the composition of two ione-to-one functions is also one-to-one.
