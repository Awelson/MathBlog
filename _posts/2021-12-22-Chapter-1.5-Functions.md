...
Proof. 1) Suppose \\( y\in F[A\cup B] \\), then by definition there exists an \\( x\in A\cup B \\) for which \\( xFy \\). To win, we must show that either \\( y\in F[A] \\) or \\( y\in F[B] \\). Since \\( x\in A\cup B \\) Kwan concludes that \\( x \\) must either be in \\( A \\) or in \\( B \\), if \\( x\in A \\), then \\( y\in F[A] \\) because \\( xFy \\), similarly, if \\( x\in B \\), then \\( y\in F[B] \\).

Suppose \\( y\in F[A] \cup F[B] \\), then we have either \\( y\in F[A] \\) or \\( y\in F[B] \\). To win, we need to show that there exists an \\( x\in A\cup B \\) for which \\( xFy \\). If \\( y\in F[A] \\), then there exists an \\( x\in A \\) for which \\( xFy \\), this is what we need to win. Alternatively, if \\( y\in F[B] \\), then there exists an \\( x\in B \\) for which \\( xFy \\), this is also a win.
