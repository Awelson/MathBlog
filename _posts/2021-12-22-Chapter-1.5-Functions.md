...
- Transformation instructions need not necessarily send one type of objects to the same type, for example, a function with domain \\( \mathbb{R} \\) and the instruction \\( f(x)=<x,x+1> \\) transforms a real number into an object of type \\( \mathbb{R} \times \mathbb{R} \\), that is, into an ordered pair of real numbers.