...
Proof. Suppose \\( <x,y_1>,<x,y_2>\in F\circ G \\). This implies the existence of \\( z_1,z_3\in G \\) and \\( z_2,z_4\in F \\) such that \\( z_1=<x,p_1>, \ z_2=<p_1,y_1> \\) and \\( z_3=<x,p_2>, \ z_4=<p_2,y_2> \\). Since \\( G \\) is a function, we have \\( p_1=p_2 \\), and again, since \\( F \\) is a function, we have \\( y_1=y_2 \\).
