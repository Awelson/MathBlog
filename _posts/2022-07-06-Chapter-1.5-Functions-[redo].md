...
It is clear that we must have \\( F\circ G(x)=F(G(x)) \\) for all \\( x \\) in the domain of \\( F\circ G \\), which begs the question: What is its domain? Well, we would need \\( x\in \text{dom }G \\) and also \\( G(x)\in \text{dom }F \\). Thus,
...
but \\( G(x)\in B \\) is satisfied by all \\( x\in A \\). Thus the domain of the composite is \\( A \\). Further, it is clear that \\( F\circ G(x)=F(G(x))\in C \\) for all \\( x\in A \\). Thus \\( F\circ G:A\to C \\) as needed. \\( \Box \\)
