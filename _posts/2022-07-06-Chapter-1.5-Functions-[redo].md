...
Proof. By definition there exists \\( z_1\in G \\) and \\( z_2\in F \\) such that \\( \text{fst}(z_1)=x, \ \text{snd}(z_2)=y \\) and \\( \text{snd}(z_1)=\text{fst}(z_2) \\). We propose \\( t=\text{snd}(z_1) \\). It is clear that \\( t\in \text{im }G\cap \text{dom }F \\), further since \\( z_1=<\text{fst}(z_1),\text{snd}(z_1)>\in G \\) we have \\( xGt \\). By a similar argument, \\( tFy \\). \\( \Box \\)
