...
> Theorem 1.5.2 \\( R \\) is injective iff \\( R^{-1} \\) is a function. Similarly, \\( R \\) is a function iff \\( R^{-1} \\) is injective.
...
Suppose \\( <y_1,x>,<y_2,x>\in R^{-1} \\). To show that \\( R^{-1} \\) is injective we need to show that \\( y_1=y_2 \\). We have \\( <x,y_1>,<x_y_2>\in R \\), but since \\( R \\) is a function we have \\( y_1=y_2 \\). The other direction follows similarly. \\( \Box \\)
...
Suppose \\( y\in \text{im }F \\), then \\( y\in \text{dom }F^{-1} \\). We use the first part of theorem to conclude that \\( (F^{-1})^{-1}(F^{-1}(y))=y \\), but since \\( (F^{-1})^{-1}=F \\) we have what we need. \\( \Box \\)
