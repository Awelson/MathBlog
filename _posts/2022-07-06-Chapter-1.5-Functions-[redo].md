...
Essentially the restriction of \\( R \\) on \\( A \\) is obtained by throwing out ordered pairs in \\( R \\) whose first coordinate isn't in \\( A \\). It is clear then that we must always have \\( R\flag_A\subseteq R \\), the restriction of a relation is always a subset of the relation. Similarly, a subset of a relation is some restriction of that relation.
