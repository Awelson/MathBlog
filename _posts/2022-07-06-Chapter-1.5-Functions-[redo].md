...
> Let \\( R \\) be a relation, then \\( R \\) is injective if for any \\( y\in \text{ran }R \\) there only exists one \\( x\in \text{dom }R \\) for which \\( xRy \\) holds true. In other words: \\( x_1Ry \ \land \ x_2Ry \ \rightarrow x_1=x_2 \\).
