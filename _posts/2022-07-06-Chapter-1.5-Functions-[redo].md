...
> Theorem 1.5.9 If \\( G:A\to B \\) and \\( F:B\to C \\) then \\( F\circ G:A\to C \\) is surjective.
...
Essentially the restriction of \\( R \\) on \\( A \\) is obtained by throwing out ordered pairs in \\( R \\) whose first coordinate isn't in \\( A \\). It is clear then that we must always have \\( R\restriction_A\subseteq R \\), the restriction of a relation is always a subset of the relation. Similarly, a subset of a relation is some restriction of that relation.
...
The domain of \\( G \\) by construction is \\( \text{im }F\cup (B-\text{im }F)=B \\), and certainly \\( G(x)\in A \\) for all \\( x\in B \\). So \\( G:B\to A \\). Now we need to show that \\( G\circ F(x)=x \\) for all \\( x\in A \\). This is obvious: \\( G(F(x))=F^{-1}(F(x))=x \\) since \\( F(x)\in \text{im }F \\) and also Theorem 1.5.4.
