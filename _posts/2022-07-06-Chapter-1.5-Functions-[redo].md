...
The above formula can actually be proved, but it is quite tedious to do so. We'll just leave it at that. Another thing to note is that if \\( <x,y>\in F\circ G \\), we can conclude that there is a \\( t\in \text{im }G \cap \text{dom }F \\) such that \\( xGt \ \land \ tFy \\). The other way works as well, if we can show that there exists a \\( t \\) such that \\( xGt \ \land tFy \\), then \\( <x,y> \\) is in the composition.
