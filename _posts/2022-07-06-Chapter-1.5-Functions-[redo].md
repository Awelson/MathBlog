...
> Theorem 1.5.5 If \\( G \\) and \\( F \\) are functions, then \\( F\circ G \\) is also a function. Additionally, if it is known that \\( G:A\to B \\) and \\( F:B\to C \\), then F\circ G:A\to C is such that \\( F\circ G(x)=F(G(x)) \\).

Proof. Suppose \\( <x,y_1>,<x,y_2>\in F\circ G \\). This implies the existence of a \\( z_1,z_3\in G \\) and \\( z_2,z_4\in F \\) such that \\( z_1=<x,p_1>, \ z_2=<p_1,y_1> \\) and \\( z_3=<x,p_2>, \ z_4=<p_2,y_2> \\). Since \\( G \\) is a function, we have \\( p_1=p_2 \\), and again, since \\( F \\) is a function, we have \\( y_1=y_2 \\).
