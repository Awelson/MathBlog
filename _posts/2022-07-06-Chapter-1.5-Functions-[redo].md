...
Essentially the restriction of \\( R \\) on \\( A \\) is obtained by throwing out ordered pairs in \\( R \\) whose first coordinate isn't in \\( A \\). We also have that if \\( f \\) is a function and \\( C\subseteq \text{dom }f \\) then \\( f\restriction_C(x)=f(x) \\) for all \\( x\in C \\). It is clear then that we must always have \\( R\restriction_A\subseteq R \\), the restriction of a relation is always a subset of the relation. Similarly, a subset of a relation is some restriction of that relation.
