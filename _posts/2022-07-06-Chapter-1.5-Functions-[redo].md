...
Suppose \\( <y_1,x>,<y_2,x>\in R^{-1} \\). To show that \\( R^{-1} \\) is injective we need to show that \\( y_1=y_2 \\). We have \\( <x,y_1>,<x_y_2>\in R \\), but since \\( R \\) is a function we have \\( y_1=y_2 \\). 
