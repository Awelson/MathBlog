...
Now suppose that \\( G:A\to B \\) and \\( F:B\to C \\). If we want \\( \text{dom }F\circ G=A \\), we would need it so that for any \\( z_1\in G \\), we can find a \\( z_2\in F \\) such that \\( \text{snd}(z_1)=\text{fst}(z_2) \\). 
