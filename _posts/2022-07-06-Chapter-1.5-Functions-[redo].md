...
> Theorem 1.5.7 The composition of injective relations is again injective.
...
> Theorem 1.5.8 If \\( G:A\to B \\) and \\( F:B\to C \\) then \\( F\circ G:A\to C \\) is surjective.
...
\\[ R\flag_A=\\{ z\in R \ \vert \ \text{fst}(z)\in A \\} \\]

Essentially the restriction of \\( R \\) on \\( A \\) is obtained by throwing out ordered pairs in \\( R \\) whose first coordinate isn't in \\( A \\). It is clear then that we must always have \\( R\flag_A\subseteq R \\), the restriction of a relation is always a subset of the relation.

> Theorem 1.5.9 If \\( R' \\) is an injective relation and \\( R\subseteq R' \\), then \\( R \\) is also an injective relation.
...
This theorem tells us that the restriction of an injective relation is again injective.
