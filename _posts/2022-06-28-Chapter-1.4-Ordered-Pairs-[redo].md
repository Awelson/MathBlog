...
Let us prove a rather obvious fact. If \\( f \\) is a function, then \\( f(x)\in \text{ran }f \\) for all \\( x\in \text{dom }f \\). Suppose \\( x\in \text{dom }f \\), then \\( <x,f(x)>\in f \\) and \\( \text{snd}(<x,f(x)>)=f(x)\in \text{ran }f \\).
...
Proof. Showing that \\( f:A\to C \\) means three things: showing that \\( f \\) is a function, showing that it has domain \\( A \\) and showing that \\( \text{ran }f\subseteq C \\). The first two are obvious. For the third, notice that since \\( f \\) has codomain \\( B \\), we have \\( \text{ran }f\subseteq B\subseteq C \\).
