...
This is a difficult concept to wrap around. The way I like to think about it is, if something like \\( f:A\to B \\) shows up this means: \\( f \\) is a function along with the added info that \\( \text{dom }f=A \\) and that \\( \text{ran }f\subseteq B \\) (or alternatively: \\( f(x)\in B \\) for all \\( x\in A \\)). If \\( f:A\to B \\), then the set \\( B \\) is said to be the codomain of \\( f \\), but this is misleading because the codomain doesn't depend on \\( f \\) at all, the codomain can literally be anything as long as \\( \text{ran }f \\) is a subset of it. The range of \\( f \\) is in this sense the smallest possible codomain of \\( f \\). 
