...
> Note. A function and its codomain are not bound such that whenever one mentions a function, its codomain needs to be made clear too. Mentioning the codomain of a function is optional, if that fact that \\( f(x)\in B \\) for all \\( x \\) in the domain is necessary in some proof or explanation, then the writer will make it clear that \\( f \\) has codomain \\( B \\), otherwise there is no need to mention the codomain.
