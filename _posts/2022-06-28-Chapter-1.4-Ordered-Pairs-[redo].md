...
\\[ \text{dom }\varnothing= \\{ x\in A\times B \ \vert \ \exists z\in \varnothing. \ \text{fst}(z)=x \\}=\varnothing \\]

Similarly, \\( \text{ran }\varnothing=\varnothing \\). So indeed \\( \varnothing:\varnothing\to \varnothing \\).  
