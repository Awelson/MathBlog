...
> Definition. A sequence \\( (x_n)\subseteq M \\) is said to converge in the metric space \\( M \\) if there is an \\( x\in M \\) such that for any \\( \epsilon>0 \\), there exists an \\( N\in \mathbb{N} \\) such that 
