...
Proof. Say Sal chooses an \\( \epsilon>0 \\), Kwan needs to come up with a response, here's how: Given the number \\( \epsilon/2 \\), Kwan has the strategy to come up with the numbers \\( N_1,N_2 \\) which satisfy \\( d(x_n,x)<\epsilon/2 \\) for all \\( n\geq N_1 \\) and \\( d(y_n,y)<\epsilon/2 \\) for all \\( n\geq N_2 \\). Thus, if \\( N=\text{max}\\{N_1,N_2\\} \\), then it must be that \\( d(x_N,x)<\epsilon/2 \\) and \\( d(y_N,y)<\epsilon/2 \\). By triangle inequality, the following is true for all \\( n\geq N \\):

\\[ d(x_n,y_n)\leq d(x_n,x)+d(x,y)+d(y_n,y)<d(x,y)+\epsilon, \newline
d(x,y)\leq d(x,x_n)+d(x_n,y_n)+d(y_n,y)<d(x_n,y_n)+\epsilon. \\]

This means that \\( \|z_n-d(x,y)\|<\epsilon \\) for all \\( n\geq N \\). Thus, all Kwan needs to do is to respond with maximum between \\( N_1,N_2 \\). \\( \Box \\)

You may find it strange that Kwan is able to respond with the numbers \\( N_1,N_2 \\) when given \\( \epsilon/2 \\). But this is not strange at all, after all, we are familiar with Kwan being able to respond to any chosen \\( \epsilon>0 \\) by Sal,

	![Capture.PNG](/MathBlog/assets/Capture4.PNG)

and it makes no difference if Sal were to input his chosen \\( \epsilon \\) into a machine which divides numbers by 2, and then afterwards, give the results to Kwan. Remember, Kwan is able to respond to any positive real number, and \\( \epsilon/2 \\) is indeed a positive real number.
