...
Proof. Let \\( \pi:\mathbb{N}\to \mathbb{N} \\) be a bijection, then \\( (x_{\pi(n)}) \\) is a rearrangement of \\( (x_n) \\). Sal begins by choosing an \\( \epsilon>0 \\), now Kwan needs to find a number, \\( N_0 \\) such that \\( d(x_{\pi(n)},x)<\epsilon \\) for all \\( n\geq N_0 \\). Since \\( (x_n)\to x \\), Kwan is able to find an \\( N \\) such that \\( d(x_n,x)<\epsilon \\) for all \\( n\geq N \\). Suppose there is a number \\( N_0 \\) with the property that \\( \pi(n)\geq N \\) for all \\( n\geq N_0 \\), then it must be that \\( d(x_{\pi(n)},x)<\epsilon \\) for all \\( n\geq N_0 \\) as needed. 

What strategy can Kwan implement to find a number with this property? Firstly, if \\( N=1 \\), then Kwan chooses \\( N_0=1 \\) because \\( \pi(n)\geq 1 \\) for all \\( n\geq 1 \\) is of course true. At this point, we may assume \\( N\geq 2 \\). Kwan needs to choose an \\( N_0 \\) such that
...
- In the metric space \\( \mathbb{R} \\), a sequence \\( (x_n) \\) is bounded iff there exists a \\( a,b\in \mathbb{R} \\) such that \\( a\leq x_n\leq b \\) for all \\( n \\). This is easy to prove
