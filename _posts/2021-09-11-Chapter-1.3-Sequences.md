...
Proof. Let \\( \pi:\mathbb{N}\to \mathbb{N} \\) be a bijection, then \\( (x_{\pi(n)}) \\) is a rearrangement of \\( (x_n) \\). For any \\( \epsilon>0 \\), we know that there is an \\( N \\) for which \\( d(x_n,x)<\epsilon \\) for all \\( n\geq N \\). We will show that it is possible to find an \\( N_0 \\) for which \\( \pi(n)\geq N \\) for all \\( n\geq N_0 \\), this gives \\( d(x_{\pi(n)},x)<\epsilon \\) for all \\( n\geq N_0 \\).

We prove by contradiction, suppose there is no such \\( N_0 \\), this means:

\\[ \forall N_0, \exists n\geq N_0, \ \pi(n)<N. \\]

Let \\( f \\) map each \\( N_0 \\) to its corresponding \\( n \\). We define a sequence recursively as follows: let \\( z_1=f(1) \\) and \\( z_n=f(1+z_{n-1})) \\). Notice that \\( z_1<z_2<\cdots \\), and because \\( \pi \\) is a bijection, \\( \pi(z_1), \pi(z_2), \ldots \\), should each be distinct from one another, yet,
