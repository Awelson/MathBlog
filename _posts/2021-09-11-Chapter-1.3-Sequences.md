...
If this is the case, we say that \\( (x_n) \\) converges to \\( x \\) in the metric space \\( M \\). We will use the notation \\( \lim{(x_n)}=x \\) or \\( (x_n)\to x \\) to indicate this. If no such \\( x\in M \\) exists, then we say that the sequence is divergent. 

That is a mouthful, so we will express this in terms of a game: A sequence \\( (x_n)\subseteq M \\) converges in the metric space \\( M \\) if Kwan can develop a winning strategy for the following game:
