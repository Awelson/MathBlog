...
Proof. We will prove by contradiction. This means that if Sal chooses \\( \epsilon=d(x,y)/2 \\), Kwan is able come up with the numbers \\( N_1,N_2 \\) for which \\( d(x_n,x)<\epsilon \\) for all \\( n\geq N_1 \\) and \\( d(x_n,y)<\epsilon \\) for all \\( n\geq N_2 \\). Let \\( N=\text{max}\\{N_1,N_2\\} \\), then both \\( d(x_N,x)<\epsilon \\) and \\( d(x_N,y)<\epsilon \\) are true. By triangle inequality,
