...
This is equivalent to needing \\( n>1/\epsilon \\) for all \\( n\geq N \\). This can be achieved if \\( N>1/\epsilon \\), so that is our winning strategy, if Sal gives a number \\( \epsilon>0 \\), then all Kwan needs to do is to pick a natural number that is larger than \\( 1/\epsilon \\).