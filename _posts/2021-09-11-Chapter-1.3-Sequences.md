...
Proof. Say Sal chooses an \\( \epsilon>0 \\), Kwan needs to come up with a response, here's how: Given the number \\( \epsilon/2 \\), Kwan can come up with the numbers \\( N_1,N_2 \\) which satisfy \\( d(x_n,x)<\epsilon/2 \\) for all \\( n\geq N_1 \\) and \\( d(y_n,y)<\epsilon/2 \\) for all \\( n\geq N_2 \\). Thus, if \\( N=\text{max}\\{N_1,N_2\\} \\), then it must be that \\( d(x_N,x)<\epsilon \\) and \\( d(y_N,y)<\epsilon \\)
