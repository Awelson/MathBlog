...
Let \\( f \\) map each \\( N_0 \\) to its corresponding \\( n \\). We define a sequence recursively as follows: let \\( z_1=f(1) \\) and \\( z_n=f(1+z_{n-1})) \\). Notice that \\( z_1<z_2<\cdots \\), so \\( \pi(z_1), \pi(z_2), \ldots \\), should each be distinct from one another, yet,
