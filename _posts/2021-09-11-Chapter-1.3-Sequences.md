...
This will make sure that \\( \pi(n)\geq N \\) whenever \\( n\geq N_0 \\). It turns out that it is sufficient for \\( \pi \\) to be injective, only a slight changes to the proof is required.
