...
Proof. We will prove by contradiction, assume that for whatever \\( \epsilon>0 \\) that Sal chooses, Kwan can come up with the numbers \\( N_1,N_2 \\) for which \\( d(x_n,x)<\epsilon/2 \\) for all \\( n\geq N_1 \\) and \\( d(x_n,y)<\epsilon/2 \\) for all \\( n\geq N_2 \\). 
