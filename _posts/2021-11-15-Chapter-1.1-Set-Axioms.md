...
It is obvious that any set \\( A \\) is a subset of itself, i.e, \\( A\subset A \\). Because of this, we say that the relation '\\( \subseteq \\)' is reflexive (is '\\( = \\)' reflexive?). If for any sets \\( A,B \\) we have \\( A\subseteq B \Longleftarrow B\subseteq A \\), then we say that '\\( \subseteq \\)' is symmetric, this however is not the case. Take for example \\( A=\\{1\\} \\) and \\( B=\\{1,2\\} \\), then \\( A\subseteq B \\) is true, but \\( B\subseteq A \\) is not true.