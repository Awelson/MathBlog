...
\\[ A=B \quad \Longleftrightarrow \quad (x\in A \Leftrightarrow x\in B), \tag{1}\\]

or alternatively,

\\[ A=B \quad \Longleftrightarrow \quad (x\in A \ \Leftarrow \ x\in B \ \land \ x\in B \ \Leftarrow \ x\in A). \tag{2}\\]
...
The statement \\( x\in A \ \Leftarrow \ x\in B \\) on its own is useful. It means that every element of \\( A \\) is in \\( B \\), whenever this is true, we say that \\( A \\) is a subset of \\( B \\), notationally, it will look like \\( A\subseteq B \\). Similarly, we write \\( B\subseteq A \\) whenever \\( x\in B \ \Leftarrow \ x\in A \\). Notice then that
