...
Proof. Suppose there is a set \\( A \\) which contains everything, to complete the proof, we need to demonstrate a contradiction. By invoking the axiom of specification, with \\( S(x)=x\notin x \\), we obtain the set \\( B=\\{ x\in A \ \vert \ x\notin x \\), a set containing elements of \\( A \\) which do not contain themselves. In other words,

\\[ x\in B \quad \Longleftrightarrow \quad x\in A \ \land \ x\notin x. \tag{1} \\]

One of these statements, \\( B\in A \\) or \\( B\notin A \\), must be true. Suppose \\( B\in A \\), then it must be that either \\( B\in B \\) or \\( B\notin B \\). If \\( B\in B \\), then by (1), \\( B\notin B \\) which is a contradiction. If \\( B\notin B \\), then by (1) again we have \\( B\in B \\), another contradiction. Thus, \\( B\in A \\) is not possible.

This means that \\( B\notin A \\) must be true. But, \\( A \\) is supposed to contain everything, yet, here we have an object, \\( B \\), that is not inside \\( A \\). This is a contradiction, thus there really is no set which contains everything. \\( \Box \\) 
