...
A relation can be thought of as 2-variable function, the inputs can be of any type as long as they are the same, the output is boolean, either true/false. For example, \\( = \\) is a relation between sets, we input two sets, ask whether or not they are equal, and we'll get either true or false for an output.
