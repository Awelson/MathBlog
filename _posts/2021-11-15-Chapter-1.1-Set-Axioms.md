...
Proof. Sal picks some element \\( x\in A \\) and gives it to Kwan, in order to win, Kwan must show that \\( x\in C \\). Kwan knows that \\( A\subseteq B \\), so this must mean that \\( x\in B \\), but he is also aware that \\( B\subseteq C \\), so this must mean that \\( x\in C \\).