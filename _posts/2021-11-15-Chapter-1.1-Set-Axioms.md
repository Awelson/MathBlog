...
\\[ A=B \quad \Longleftrightarrow \quad \forall x\in A, \ x\in B \ \and \ \forall x\in B, x\in A. \\]

The right hand side essentially means that every element of \\( A \\) has to be in \\( B \\) **and** every element of \\( B \\) has to be in \\( A \\). This is equivalent to \\( A \\) and \\( B \\) having the exact same elements.