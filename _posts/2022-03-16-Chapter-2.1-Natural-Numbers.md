...
To show that \\( T \\) is inductive, we will need to show two things: that \\( 0\in T \\) (this is known as the base case) and that for any \\( n\in T \\) we must have \\( n^+\in T \\) (this is known as the inductive step).
...
Proof. Note, if \\( T \\) is inductive, then \\( T=\omega \\). Now suppose Sal chooses an \\( n \\) from \\( \omega \setminus \\{0\\} \\). Kwan needs to be able to show that \\( P(n) \\). Kwan notices that there must exist a \\( b\in omega \\) such that \\( b^+=n \\) (we proved this already). But since \\( b\in\omega=T \\), we know that \\( P(b^+)=P(n) \\) is true. \\( \Box \\)

Letting \\( T=\\{ n\in \omega \ \vert \ P(n^+) \\} \\) has the effect of switching the base case from 0 to 1. Why? The base case is to show that \\( 0\in T \\), i.e, that \\( P(0^+)=P(1) \\) is true.
