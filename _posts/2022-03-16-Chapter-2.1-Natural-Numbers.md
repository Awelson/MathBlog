...
Proof. Note, if \\( T \\) is inductive, then \\( T=\omega \\). Now suppose \\( n\in \omega \setminus \\{0\\} \\). Our goal is to show that \\( P(n) \\) holds. Notice that there must exist a \\( b\in \omega \\) such that \\( b^+=n \\) (we proved this already). But since \\( b\in T \\) (because \\( \omega=T \\)), we know that \\( P(b^+)=P(n) \\) is true. \\( \Box \\)
...
> A prerequisite before we get into what the postulates are: If \\( S \\) is a function and \\( A\subseteq \text{dom }S \\), then we say that \\( A \\) is closed under \\( S \\) if whenever \\( x\in A \\) we have \\( S(x)\in A \\) (this is equivalent to saying that \\( S[A]\subseteq A \\)).
