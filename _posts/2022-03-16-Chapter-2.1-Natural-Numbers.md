...
We call it the infinity axiom because an inductive set has (at minimum) infinite members, it contains:

\\[ \varnothing, \quad \varnothing^+, \quad \varnothing^{++}, \quad \cdots \\]

and so on. With this, we give a formal definition for numbers as follows:

> Definition. A set, \\( n \\), is a number if it is contained in every inductive set. In other words: \\( \forall A \ [A\text{ is inductive } \land \ n\in A] \\).
