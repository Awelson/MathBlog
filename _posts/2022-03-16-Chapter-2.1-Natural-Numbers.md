...
To show that \\( T \\) is inductive, we will need to show two things: that \\( 0\in T \\) (this is known as the base case) and that for any \\( k\in T \\) we must have \\( k^+\in T \\) (this is known as the inductive step).
...
Proof. Note, if \\( T \\) is inductive, then \\( T=\omega \\). Now suppose \\( n\in \omega - \\{0\\} \\). Our goal is to show that \\( P(n) \\) holds. Notice that there must exist a \\( b\in \omega \\) such that \\( b^+=n \\) (we proved this already). But since \\( b\in T \\) (because \\( \omega=T \\)), we know that \\( P(b^+)=P(n) \\) is true. \\( \Box \\)
...
Of course we can use other base cases, for example \\( n=4 \\), as well. We just have to extend our reasoning.
...
Proof. Pick an \\( x\in \bigcup (A\cup B) \\), our goal is to show either of two things: that \\( x\in \bigcup A \\) or that \\( x\in \bigcup B \\). We know that there must exist a \\( y\in A\cup B \\) such that \\( x\in y \\). Suppose \\( y\in A \\), then \\( x\in y\in A \\) so \\( x\in \bigcup A \\). If instead \\( y\in B \\), then \\( x\in y\in B \\) so \\( x\in \bigcup B \\).

The reverse is similar enough. \\( \Box \\)
...
Proof. If we form \\( T=\\{ n\in \omega \ \vert \ n \text{ is transitive } \\} \\), then it suffices to show that \\( T \\) is inductive. First, \\( 0\in T \\) because \\( \varnothing \\) is trivially transitive. Now suppose \\( k\in T \\). We need to show that \\( k^+ \\) is transitive, to do this, we show that \\( \bigcup k^+\subseteq k^+ \\). This is obvious because
