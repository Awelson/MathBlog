...
Proof. We prove by contradiction. Suppose \\( A\subseteq \omega \\) is non-empty but has no least element, then a contradiction will arrise if we can show that \\( A=\varnothing \\). To do this, we prove the following lemma:
