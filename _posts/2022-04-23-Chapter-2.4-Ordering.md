...
\\[ m\in n \quad \Longrightarrow m+n\cdot k\in n+n\cdot k=n\cdot k^+ \\]

so \\( m\cdot k^+\in n\cdot k^+ \\) as needed.

For the reverse: Suppose \\( m\cdot p\in n\cdot p \\). Then we obviously cannot have \\( m=n \\). We also cannot have \\( n\in m \\) because the forward direction of Theorem 2.4.7 gives \\( n\cdot p\in m\cdot p \\) and \\( m\cdot p\in n\cdot p \\) by hypothesis. Thus, the only option left is \\( m\in n \\). \\( \Box \\)
