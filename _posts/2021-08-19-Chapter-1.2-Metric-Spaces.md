...
- \\( d(x,x)=0, \ \\) for any \\( x\in M. \\) Conversely, if \\( x,y \\) are two points from \\( M \\) and \\( d(x,y)=0 \\), then it must be that \\( x=y \\).
...
\\[ d(y,z)\leq d(y,x)+d(x,z)\Longrightarrow d(x,y)\geq d(y,z)-d(x,z)\Longrightarrow -d(x,y)\leq d(x,z)-d(x,y). \\]
