...
Now we need to develop a strategy that guarantees Sal's victory. Here is his strategy: Sal will randomly choose a point \\( a\in A \\) and a point \\( b\in B \\), and then notice that
...
- Let \\( M \\) be a metric space and \\( S\subseteq M \\) be a metric subspace. Show that if \\( S \\) is unbounded, then so is \\( M \\).
