...
- In chapter 1.1, we provided an alternative description of boundedness for subsets of \\( \mathbb{R} \\). Show that the two descriptions are equivalent, i.e, that a subset \\( A \\) of \\( \mathbb{R} \\) is bounded (use the old definition) if and only if the metric subspace \\( A\subseteq \mathbb{R} \\) has finite diameter.
