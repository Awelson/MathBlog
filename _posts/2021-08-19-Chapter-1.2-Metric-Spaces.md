...
**Example.** Consider \\( \mathbb{R}^2 \\), we are also familiar with a method of measuring distances between points of this set. Via pythagoras, we derive a metric \\( d:\mathbb{R}^2\times \mathbb{R}^2\to \mathbb{R} \\), where if \\( x=(x_1,x_2) \\) and \\( y=(y_1,y_2) \\) are arbitrary points of \\( \mathbb{R}^2 \\), then \\( d(x,y)=\sqrt{\|x_1-x_2\|^2+\|y_1-y_2\|^2} \\).
