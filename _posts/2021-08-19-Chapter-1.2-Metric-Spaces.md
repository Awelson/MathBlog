...
![Capture.PNG](/MathBlog/assets/Capture.PNG)
Measuring the distance between \\( x \\) and \\( y \\) on the left figure with the metric \\( p \\) should yield the same result as measuring the distance between \\( x \\) and \\( y \\) on the right figure with the metric \\( d \\).
