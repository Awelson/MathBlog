...
The diameter of a metric space gives us an idea of how big a metric space is. The diameter is defined as follows:
...
- In chapter 1.1, we provided an alternative description of boundedness for subsets of \\( \mathbb{R} \\) (Definition 1.1.6). Show that the two descriptions are equivalent, i.e, that a subset \\( A \\) of \\( \mathbb{R} \\) is bounded (use the old definition) if and only if the metric subspace \\( A\subseteq \mathbb{R} \\) has finite diameter.

> Let \\( M \\) be a metric space, to show that \\( M \\) is bounded, we don't actually need to know the value of \\( \text{diam}(M)=\text{sup }A \\) (where \\( A=\\{d(x,y) \ \vert \ x,y\in M\\} \\)). By the least upper bound property, it suffices to show that \\( A \\) is bounded above in order to show that \\( \text{sup }A \\) exists (and is finite), in other words, it suffices to show that there exists some real number, \\( r \\), such that \\( d(x,y)<r \\) for all \\( x,y\in M \\). Further, this \\( r \\) value satisfies \\( \text{diam}(M)\leq r \\).
...
Now we need to develop a strategy that guarantees Sal's victory no matter how many times the game is played. Here is the strategy: Sal will randomly choose a point \\( a\in A \\) and a point \\( b\in B \\), and then notice that

\\[ d(m_1,m_2)\leq d(m_1,a)+d(a,b)+d(b,m_2)\leq \alpha+\beta+d(a,b), \quad \forall m_1\in A, \forall m_2\in B. \\]

Sal picks the number \\( \alpha+\beta+d(a,b) \\) knowing that his victory is guaranteed if Kwan picks one point from \\( A \\) and another from \\( B \\). If Kwan is aware of this, he may instead pick both of his points from \\( A \\), unfortunately he'll still lose because the distance between those two points \\( \leq\alpha \\). For similar reasons, Kwan also loses if he picks both of his points from \\( B \\). Thus, Sal wins no matter what. \\( \Box \\)
...
- By induction, if \\( M=\displaystyle{\bigcup_{n=1}^k{U_n}} \\), \\( k \\) is finite, and each \\( U_n \\) has finite diameter, then \\( M \\) also has finite diameter.
...
> Definition. Let \\( M,N \\) be metric spaces, we say that \\( f:M\to N \\) is an isometry from \\( M \\) to \\( N \\) if
