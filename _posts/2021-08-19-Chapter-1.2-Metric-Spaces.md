...
Proof. Let \\( \text{diam}(A)=\alpha \\) and \\( \text{diam}(B)=\beta \\). Pick a point \\( a'\in A \\) and a point \\( b'\in B \\) randomly. We will show that

\\[ d(m_1,m_2)\leq \alpha+\beta+d(a',b'), \quad \text{for all }m_1,m_2\in M. \\]

Notice that this is obviously true if both \\( m_1,m_2\in A \\) or both \\( m_1,m_2\in B \\). There is one last possibility to consider (what is it?). By the triangle inequality,

\\[ d(m_1,m_2)\leq d(m_1,a')+d(a',b')+d(b',m_2)\leq \alpha+\beta+d(a',b'), \quad \forall m_1\in A, m_2\in B. \\]

So this case is taken care of as well. \\( \Box \\)
