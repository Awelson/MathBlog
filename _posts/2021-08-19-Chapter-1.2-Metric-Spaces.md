...
- Show that if \\( (M,d) \\) is a metric space with \\( d \\) being the discrete metric, then it is bounded.
- In chapter 1.1, we provided an alternative description of boundedness for subsets of \\( \mathbb{R} \\). Show that these two descriptions are in fact equivalent.
