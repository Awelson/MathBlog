...
> Let \\( M \\) be a metric space, to show that \\( M \\) is bounded, we don't actually need to know the value of \\( \text{diam}(M)=\text{sup }A \\) \\( (A=\\{d(x,y) \ \vert \ x,y\in M\\}) \\)). By the least upper bound property, it suffices to show that \\( A \\) is bounded above, this implies that \\( \text{sup }A \\) exists (and is finite).
