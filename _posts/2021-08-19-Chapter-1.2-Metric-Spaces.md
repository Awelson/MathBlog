...
Sal picks the number \\( r=\alpha+\beta+d(a,b) \\), the game can then continue in one of three ways: 1) Kwan chooses one point from \\( A \\) and another from \\( B \\), 2) Kwan chooses both points from \\( A \\), 3) Kwan chooses both points from \\( B \\). 
