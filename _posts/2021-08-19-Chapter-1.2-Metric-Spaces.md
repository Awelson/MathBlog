...
The pairing of \\( S\subseteq M \\) with an inherited metric from \\( M \\) creates what is known as a metric subspace of \\( (M,d) \\). Conversely, a metric subspace of \\( (M,d) \\) is a subset of \\( M \\) paired with an inherited metric from \\( M \\).

**Example.** Consider the metric space \\( \mathbb{R} \\), since \\( \mathbb{N}\subseteq \mathbb{R} \\), we can convert \\( \mathbb{N} \\) into a metric subspace of \\( \mathbb{R} \\) by pairing it with \\( d\restriction_{\mathbb{N}} \\), with \\( d \\) being the euclidean 1-metric. 
