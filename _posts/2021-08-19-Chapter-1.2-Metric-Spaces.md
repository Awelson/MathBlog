...
To prove this, let us first show that 1 is an upper bound for this set, i.e,

\\[ |x-y|\leq 1, \quad \forall x,y\in (0,1). \\]

Let \\( x,y\in (0,1) \\) be arbitrary. Verify that \\( d(x,0.5),d(y,0.5)\leq 0.5 \\), where \\( d \\) is the euclidean 1-metric. Now the triangle inequality gives

\\[ d(x,y)\leq d(x,0.5)+d(0.5,y)\leq 1 \\].
