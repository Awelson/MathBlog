...
> In continuation to the example above, when we state something along the lines of 'Let \\( S \\) be a metric subspace of \\( M \\)', a metric on \\( S \\) is not explicitly mentioned, this is because the reader is expected to have an automatic understanding that the metric on \\( S \\) needs to be one that is inherited from \\( M \\).
