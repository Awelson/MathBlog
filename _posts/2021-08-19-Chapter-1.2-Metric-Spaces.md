...
![Capture.PNG](/MathBlog/assets/Capture2.PNG)
...
> In line with the example above, when we state something along the lines of 'Let \\( S \\) be a metric subspace of \\( M \\)', a metric on \\( S \\) is not explicitly mentioned, this is because the reader is expected to have an automatic understanding that the metric on \\( S \\) needs to be one that is inherited from \\( M \\). 

> We will often use the symbol/name for the distance function on a subspace as is used for the distance function on the containing space, this is to avoid a notational mess.
...
To prove this, let us first show that 1 is an upper bound for this set: Let \\( x,y\in (0,1) \\) be arbitrary. Verify that both \\( d(x,0.5)\leq 0.5 \\) and \\( d(y,0.5)\leq 0.5 \\), where \\( d \\) is the euclidean 1-metric. Now the triangle inequality gives

\\[ \|x-y\|=d(x,y)\leq d(x,0.5)+d(0.5,y)\leq 1 \\].

so we are done. Now let us show that if \\( c<1 \\), then \\( c \\) cannot be an upper bound for the set. We may assume that \\( 0<c<1 \\) and ignore the other cases (why?). Now we need to show that there exists \\( x,y\in (0,1) \\) such that \\( c<d(x,y) \\). 

Verify that there exists an \\( \epsilon>0 \\) such that \\( c+\epsilon<1 \\). Therefore it must be that \\( \epsilon/2\in (0,1) \\). Now see that

\\[ c<d(c+\epsilon,\epsilon/2)=c+\epsilon/2. \\]

So we are done, the diameter of the metric subspace \\( (0,1) \\) is equal to 1.
