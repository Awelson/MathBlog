...
It is obvious that \\( p \\) is a metric on \\( S \\), the (four) properties satisfied by \\( d \\) simply carries over to its restriction, \\( p \\). Our point is: a metric on \\( M \\) automatically defines a metric on a subset \\( S\subseteq M \\) via restriction, such a metric is said to be one that is 'inherited' from \\( M \\).

Any subset of \\( M \\) can be paired with an inherited metric to create a metric space, this results in what is known as a metric subspace of \\( M \\).
